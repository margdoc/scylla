...
public:
    ...
    static future<> do_with(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in) {
        ...
        return seastar::async([cfg_in = std::move(cfg_in), func] {
            ...
            qp.start(std::ref(proxy), std::ref(forward_service), std::move(local_data_dict), std::ref(mm_notif), std::ref(mm), qp_mcfg, std::ref(cql_config), auth_prep_cache_config, nullptr).get();
