          ...
          try {
            ...
            if (cfg->check_experimental(db::experimental_features_t::RAFT)) {
                ...
                if (cfg->check_experimental(db::experimental_features_t::GROUP0_TABLES)) {
                    startlog.error("Bad configuration: RAFT feature has to be enabled if GROUP0_TABLES is enabled");
                    throw bad_configuration_error();
                }
            }
            ...
            qp.start(std::ref(proxy), std::ref(forward_service), std::move(local_data_dict), std::ref(mm_notifier), std::ref(mm), qp_mcfg, std::ref(cql_config), std::move(auth_prep_cache_config), cfg->check_experimental(db::experimental_features_t::GROUP0_TABLES) ? &group0_client : nullptr).get();
            // #293 - do not stop anything
