...
struct experimental_features_t {
    // NOTE: RAFT and GROUP0_TABLES features are not enabled via `experimental` umbrella flag.
    // These options should be enabled explicitly.
    // RAFT feature has to be enabled if GROUP0_TABLES is enabled.
    enum class feature { UNUSED, UDF, ALTERNATOR_STREAMS, ALTERNATOR_TTL, RAFT,
            GROUP0_TABLES, KEYSPACE_STORAGE_OPTIONS };
