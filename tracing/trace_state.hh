...
public:
    using cache_counter_t = uint32_t;
...
public:
    ...
    void increment_cache_counter(cache_counter_t count) {
...
inline void increment_cache_counter(const trace_state_ptr& p, trace_state_ptr::cache_counter_t count) {
    if (p.has_opentelemetry()) {
        p.get_opentelemetry_ptr()->increment_cache_counter(count);
    }
}
