...
class topology_coordinator {
    ...
    // Each mutation is sent in separate raft command.
    // It takes group0_guard and if the number of mutations is greater than one, the guard is dropped
    // and a new one is created and returned, otherwise the old one will be returned.
    // Commands are sent sequentially and unguarded (guard used for sending the last mutation will
    // guarantee that the term hasn't been changed).
    // Returns the generation's UUID, guard and last mutation which
    ...
    //
    // In a multi-DC cluster with 100ms latencies between DCs, which means ~200ms for each command,
    // because we first perform a barrier in start_operation and then commit the command. Thus this operation will
    // take about 1.6s which is incomparably smaller than bootstrapping operation.
