...
public:
    ...
    topology_mutation_builder& set_global_topology_request(std::optional<global_topology_request>);
...
topology_mutation_builder& topology_mutation_builder::set_global_topology_request(
        std::optional<global_topology_request> value) {
    if (value) {
        _m.set_static_cell("global_topology_request", fmt::format("{}", *value), _ts);
    } else {
    ...
    future<> exec_direct_command_helper(raft::server_id id, const raft_topology_cmd& cmd) {
        ...
        if (!ip) {
            ...
            co_await coroutine::exception(std::make_exception_ptr(
                    std::runtime_error(fmt::format("no ip address mapping for {}", id))));
        }
        ...
        if (result.status == raft_topology_cmd_result::command_status::fail) {
            co_await coroutine::exception(std::make_exception_ptr(
                    std::runtime_error(fmt::format("failed status returned from {}/{}", id, *ip))));
        }
    };
    ...
    future<bool> handle_topology_transition(group0_guard guard) {
        ...
        switch (*tstate) {
            case topology::transition_state::commit_cdc_generation: {
                ...
                auto str = fmt::format("committed new CDC generation, ID: {}", cdc_gen_id);
            ...
            case topology::transition_state::publish_cdc_generation: {
                ...
                auto str = fmt::format("published CDC generation, ID: {}", curr_gen_id);
                co_await update_topology_state(std::move(guard), {builder.build()}, std::move(str));
            }
                break;
...
future<> storage_service::raft_check_and_repair_cdc_streams() {
    ...
    // FIXME: it might not need repair and it won't change (?)
