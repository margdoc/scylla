...
{
    auto response = make_unique_response(stream, cql_binary_opcode::ERROR, tr_state);
...
{
    return make_unique_response(stream, cql_binary_opcode::READY, tr_state);
}
...
{
    auto response = make_unique_response(stream, cql_binary_opcode::AUTHENTICATE, tr_state);
...
std::unique_ptr<cql_server::response> cql_server::connection::make_auth_success(int16_t stream, bytes b, const tracing::trace_state_ptr& tr_state) const {
    auto response = make_unique_response(stream, cql_binary_opcode::AUTH_SUCCESS, tr_state);
...
std::unique_ptr<cql_server::response> cql_server::connection::make_auth_challenge(int16_t stream, bytes b, const tracing::trace_state_ptr& tr_state) const {
    auto response = make_unique_response(stream, cql_binary_opcode::AUTH_CHALLENGE, tr_state);
    response->write_bytes(std::move(b));
    return response;
}
...
std::unique_ptr<cql_server::response> cql_server::connection::make_supported(int16_t stream, const tracing::trace_state_ptr& tr_state) const
{
    ...
    auto response = make_unique_response(stream, cql_binary_opcode::SUPPORTED, tr_state);
...
make_result(int16_t stream, messages::result_message& msg, const tracing::trace_state_ptr& tr_state,
    ...
    auto response = make_unique_response(stream, cql_binary_opcode::RESULT, tr_state);
...
{
    auto response = make_unique_response(-1, cql_binary_opcode::EVENT, tracing::trace_state_ptr());
